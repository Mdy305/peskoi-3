...
export default function SaasDashboard() {
  ...
  const { data: transactions = [] } = useQuery({
    ...
    queryFn: () =>
      base44.entities.WidgetTransaction.filter({ status: "completed" }),
  });

  const stats = {
    total_clients: clients.length,
    active_clients: clients.filter((c) => c.subscription_status === "active")
      .length,
    total_revenue: clients.reduce(
      (sum, c) => sum + (c.total_revenue_generated || 0),
      0,
    ),
    peskoi_revenue: clients.reduce(
      (sum, c) => sum + (c.peskoi_commission_earned || 0),
      0,
    ),
    total_bookings: clients.reduce(
      (sum, c) => sum + (c.total_bookings || 0),
      0,
    ),
    total_conversations: clients.reduce(
      (sum, c) => sum + (c.total_conversations || 0),
      0,
    ),
    avg_conversion:
      clients.length > 0
        ? (
            clients.reduce((sum, c) => sum + (c.conversion_rate || 0), 0) /
            clients.length
          ).toFixed(1)
        : 0,
  };
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-20">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-12 sm:mb-16">
          ...
          >
            ...
            <p className="text-2xl sm:text-3xl font-light mb-1 sm:mb-2">
          ...
          >
            ...
            <p className="text-2xl sm:text-3xl font-light mb-1 sm:mb-2">
              ${Math.round(stats.peskoi_revenue)}
            </p>
            ...
            {clients.map((client, index) => (
              ...
              >
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-4">
                  ...
                  <span
                    className={`text-[10px] sm:text-xs px-2 py-1 border self-start ${
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 text-xs">
                  <div>
                    ...
                    <p className="text-white/80">
                      ${Math.round(client.total_revenue_generated || 0)}
                    </p>
                  </div>
                  <div>
                    <p className="text-white/40 mb-1">Commission</p>
                    <p className="text-white/80">
                      ${Math.round(client.peskoi_commission_earned || 0)}
                    </p>
                  </div>
                  <div>
