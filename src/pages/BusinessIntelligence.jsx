      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12 sm:py-20">
        ...
        {data && !isAnalyzing && !isLoading && (
          ...
          >
            ...
            {/* Service Trends */}
            {data.analysis?.serviceTrends &&
              data.analysis.serviceTrends.length > 0 && (
                <div>
                  <p className="text-xs text-white/30 tracking-[0.15em] uppercase mb-6">
                    Service trends
                  </p>
                  <div className="space-y-4">
                    {data.analysis.serviceTrends.map((trend, index) => (
                      <div
                        key={index}
                        className="border-b border-white/[0.05] pb-6"
                      >
            ...
            {/* Stylist Insights */}
            {data.analysis?.stylistInsights &&
              data.analysis.stylistInsights.length > 0 && (
                <div>
                  <p className="text-xs text-white/30 tracking-[0.15em] uppercase mb-6">
                    Stylist performance
                  </p>
            ...
            {/* Revenue Gaps */}
            {data.analysis?.revenueGaps &&
              data.analysis.revenueGaps.length > 0 && (
                <div>
                  <p className="text-xs text-white/30 tracking-[0.15em] uppercase mb-6">
            ...
            {data.analysis?.strategicRecommendations &&
              data.analysis.strategicRecommendations.length > 0 && (
                <div>
                  <p className="text-xs text-white/30 tracking-[0.15em] uppercase mb-6">
                    Strategic recommendations
                  </p>
                  <div className="space-y-4">
                    {data.analysis.strategicRecommendations.map(
                      (rec, index) => (
                        <div
                          key={index}
                          className="border-b border-white/[0.05] pb-6"
