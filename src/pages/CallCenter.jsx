...
export default function CallCenter() {
  ...
  const filteredCalls = useMemo(() => {
    ...
    return callLogs.filter((call) => {
      ...
      const dateMatch =
        callDate >= dateRange.start && callDate <= dateRange.end;
      const statusMatch =
  ...
  const metrics = useMemo(() => {
    ...
    const conversionRate =
      completed.length > 0 ? Math.round((booked / completed.length) * 100) : 0;
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full ${liveCalls.length > 0 ? "bg-green-400 animate-pulse" : "bg-white/20"}`}
                  />
              ...
              <div className="space-y-2">
                {filteredCalls.map((call, index) => (
                  <motion.div
                    ...
                    onClick={() =>
                      setSelectedCall(
                        selectedCall?.id === call.id ? null : call,
                      )
                    }
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-white/40">
                            {new Date(call.created_date).toLocaleDateString()} •{" "}
                      ...
                      <div className="flex items-center gap-3">
                        {call.outcome && (
                          <span
                            className={`text-xs uppercase tracking-wider ${getOutcomeColor(call.outcome)}`}
                          >
                            {call.outcome}
                          </span>
                        )}
                            ...
                            <div>
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setIsPlaying(
                                    isPlaying === call.id ? null : call.id,
                                  );
                                }}
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="border border-white/[0.08] p-6">
                  ...
                  <div className="space-y-4">
                    {["bokad", "information", "ingen_åtgärd", "misslyckad"].map(
                      (outcome) => {
                        const count = filteredCalls.filter(
                          (c) => c.outcome === outcome,
                        ).length;
                        const percentage =
                          filteredCalls.length > 0
                            ? Math.round((count / filteredCalls.length) * 100)
                            : 0;
