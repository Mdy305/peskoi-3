...
export default function Campaigns() {
  ...
  const executeCampaignMutation = useMutation({
    mutationFn: (id) =>
      base44.functions.invoke("executeCampaign", { campaign_id: id }),
    onSuccess: () => queryClient.invalidateQueries(["campaigns"]),
  });
            ...
            {dynamicSegments?.segments && (
              <div className="space-y-6 mb-12">
                <p className="text-xs text-white/40 tracking-wide uppercase mb-4">
                  AI Dynamic Segments ({dynamicSegments.total_clients} clients •
                  ${Math.round(dynamicSegments.total_analyzed_ltv)} LTV)
                </p>
                      ...
                      <div className="border-t border-white/[0.05] pt-4 mt-4 space-y-3">
                        ...
                        <div className="bg-white/[0.02] p-3 border-l-2 border-white/20">
                          ...
                          <p className="text-sm text-white/80 italic">
                            "{seg.campaign_strategy.sample_message}"
                          </p>
                        </div>
                        ...
                        {seg.expected_outcomes && (
                          <div className="text-xs text-white/40">
                            <p>
                              Est. conversion:{" "}
                              {seg.expected_outcomes.estimated_conversion}%
                            </p>
                            <p>
                              Est. revenue: $
                              {Math.round(
                                seg.expected_outcomes.estimated_revenue,
                              )}
                            </p>
                          </div>
                        )}
                      ...
                      onChange={(e) =>
                        setSegmentForm({
                          ...
                          criteria: {
                            ...
                            days_since_last_visit_min: parseInt(e.target.value),
                          },
                        })
                      }
                      className="w-full bg-transparent border-b border-white/[0.08] pb-2 text-sm focus:outline-none"
              ...
              <div key={seg.id} className="border-b border-white/[0.08] pb-6">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-white/40">
                    {seg.estimated_size || 0} clients
                  </span>
                </div>
                <p className="text-xs text-white/40">
                  {seg.criteria.loyalty_tier} tier,{" "}
                  {seg.criteria.days_since_last_visit_min}+ days since visit
                </p>
              </div>
            ))}
          </div>
              ...
              <div className="border border-white/[0.08] p-6 space-y-4">
                ...
                {predictTimingMutation.data?.success && (
                  <div className="bg-white/[0.03] p-4 text-xs space-y-2">
                    <p className="text-white/40 tracking-wide uppercase">
                      AI Recommendation
                    </p>
                    <p className="text-white/80">
                      Send on:{" "}
                      {predictTimingMutation.data.prediction.next_send_friendly}
                    </p>
                    <p className="text-white/80">
                      Channel:{" "}
                      {
                        predictTimingMutation.data.prediction
                          .recommended_channel
                      }
                    </p>
                    <p className="text-white/60">
                      {predictTimingMutation.data.prediction.timing_rationale}
                    </p>
                  </div>
                )}
                ...
                <button
                  onClick={() => {
                    createCampaignMutation.mutate({
                      ...campaignForm,
                      scheduled_date:
                        predictTimingMutation.data?.prediction.next_send_time,
                    });
            ...
            {campaigns.map((camp) => (
              <div key={camp.id} className="border-b border-white/[0.08] pb-6">
                ...
                {camp.status === "draft" && (
                  ...
                  >
                    {executeCampaignMutation.isLoading
                      ? "Launching..."
                      : "Launch"}
                  </button>
                )}
