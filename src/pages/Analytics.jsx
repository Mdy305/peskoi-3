...
export default function Analytics() {
  ...
  const serviceTrends = useMemo(() => {
    ...
    bookings.forEach((b) => {
      ...
      const service =
        b.appointment_segments?.[0]?.service_variation_version?.name;
      if (service) {
  ...
  const stylistPerformance = useMemo(() => {
    ...
    completedBookings.forEach((b) => {
      ...
      const revenue =
        (b.appointment_segments?.[0]?.service_variation_version?.price_money
          ?.amount || 0) / 100;
  ...
  const clientSegments = useMemo(() => {
    ...
    const segments = {
      vip: {
        ...
        ltv: 0,
        avgVisits: 0,
        action: "Maintain exclusive experience with priority booking",
      },
      high: {
        clients: [],
        ltv: 0,
        avgVisits: 0,
        action: "Upsell premium services and loyalty program",
      },
      medium: {
        clients: [],
        ltv: 0,
        avgVisits: 0,
        action: "Increase frequency with targeted offers",
      },
      standard: {
        clients: [],
        ltv: 0,
        avgVisits: 0,
        action: "Convert to regular with first-visit incentives",
      },
    };
    ...
    Object.keys(segments).forEach((key) => {
      ...
      seg.ltv =
        seg.count > 0
          ? Math.round(
              seg.clients.reduce((sum, c) => sum + c.ltv, 0) / seg.count,
            )
          : 0;
      seg.avgVisits =
        seg.count > 0
          ? (
              seg.clients.reduce((sum, c) => sum + c.visits, 0) / seg.count
            ).toFixed(1)
          : 0;
    });
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12 sm:py-20">
        ...
        >
          {["overview", "trends", "segments", "predictions"].map(
            (tab, index) => (
              <motion.button
                key={tab}
                ...
                {Object.entries(clientSegments).map(([tier, data], index) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <p className="text-sm text-white/90 uppercase tracking-wider mb-1">
                          {tier}
                        </p>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4 pb-4 border-b border-white/[0.05]">
                      ...
                      <div>
                        ...
                        <p className="text-white/80">
                          ${Math.round(data.ltv * data.count)}
                        </p>
                      </div>
                    </div>
          ...
          {activeTab === "predictions" && (
            ...
            >
              {predictions?.success ? (
                <div className="space-y-12">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-8 mb-8">
                      <div>
                        <p className="text-2xl font-light mb-2">
                          {predictions.prediction.next_30_days_bookings
                            ?.predicted_total || "N/A"}
                        </p>
                        <p className="text-xs text-white/40">
                          Expected bookings
                      ...
                      <div>
                        <p className="text-2xl font-light mb-2">
                          $
                          {predictions.prediction.revenue_forecast?.predicted_total?.toFixed(
                            0,
                          ) || "N/A"}
                        </p>
                        <p className="text-xs text-white/40">
                    ...
                    <p className="text-xs text-white/60">
                      Trend:{" "}
                      {predictions.prediction.next_30_days_bookings?.trend ||
                        "Unknown"}
                    </p>
                  </div>
                  ...
                  {predictions.prediction.recommendations?.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-2">
                        {predictions.prediction.recommendations.map(
                          (rec, i) => (
                ...
                <p className="text-sm text-white/40">
                  Insufficient booking history for predictions
                </p>
              )}
            </motion.div>
          )}
