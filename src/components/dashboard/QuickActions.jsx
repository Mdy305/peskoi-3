...
import { Mic, Calendar, Settings, BarChart3, Zap } from "lucide-react";
...
export default function QuickActions({ businessCount }) {
  const actions = [
    ...
    {
      title:
        businessCount > 0 ? "Redigera Inställningar" : "Konfigurera Företag",
      description:
        businessCount > 0 ? "Uppdatera företagsinfo" : "Skapa ditt företag",
      icon: Settings,
      url: "Settings",
      color: "bg-purple-500",
      textColor: "text-white",
            ...
            >
              <div className="p-4 rounded-lg bg-[#0A0A0A] border border-[#2A2A2A] hover:border-[#84CC16]/50 transition-all duration-200 group">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2.5 rounded-lg ${action.color} group-hover:scale-110 transition-transform duration-200`}
                  >
