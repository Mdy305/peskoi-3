...
export default function AISettings({ business, onSave }) {
  ...
  const defaultInstructions =
    "You are a friendly and professional receptionist helping customers book appointments. Be polite, clear, and helpful. Always ask for the customer's name and phone number. Always confirm the appointment time before ending the call.";

  const defaultConfirmation =
    "Hi {name}! Your appointment for {service} is booked on {date} at {time} at {business_name}. We look forward to seeing you! Call us at {phone} if you need to change the time.";

  const defaultReminder =
    "Hi {name}! Reminder: You have an appointment for {service} tomorrow {date} at {time} at {business_name}. See you then! Call {phone} with questions.";
  ...
  useEffect(() => {
    if (business) {
      setFormData({
        ...
        confirmation_template:
          business.confirmation_template || defaultConfirmation,
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div className="flex items-center gap-2 mb-3">
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    send_customer_reminders: e.target.checked,
                  })
                }
          ...
          <div className="flex justify-end pt-2">
            <Button
              type="submit"
              className="bg-[#84CC16] hover:bg-[#84CC16]/90 text-black w-full sm:w-auto text-sm h-9"
            >
